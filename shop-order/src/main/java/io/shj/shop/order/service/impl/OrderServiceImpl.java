...
public class OrderServiceImpl implements OrderService {
    ...
    public void saveOrder(OrderParams orderParams) {
        ...
        // 查询用户信息
        User user = restTemplate.getForObject("http://localhost://localhost/shop-user/get/" + orderParams.getUserId(), User.class);
        if(user == null){
            throw new RuntimeException("未获取到用户信息："+ JSON.toJSONString(orderParams));
        }

        // 查询商品信息
        Product product = restTemplate.getForObject("http://localhost://localhost/shop-product/get/" + orderParams.getProductId(), Product.class);
        if(user == null){
